...
module.exports = {
    ...
    description: 'Update the camp player\'s link to the character sheet and token',
    ...
    async execute(client, message, args, joined, embed) {
        ...
        if ((sheet + token >= 2 && Math.abs(sheet - token) == 1) || (sheet > -1 && sheet + 1 == args.length) || (token > -1 && token + 1 == args.length))
            return message.channel.send(embed.setDescription("Please provide enough link(s)."));
